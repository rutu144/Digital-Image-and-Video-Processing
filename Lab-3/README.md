...
The implementation can use Python libraries for image handling, numerical operations, plotting, and data organization.
...
The following questions must be answered in the final report.

## Question 1

If the order of operations is changed from:
...
would the Task 4 numerical results change?

The experiment should be performed and the actual results should be reported.

The reason for the difference or similarity should also be explained, particularly with respect to linear filtering operations.

---

## Question 2

The Laplacian sharpening implementation internally uses correlation.

Would using true convolution, which flips the kernel, change the Task 2 output?

The answer should be justified using the specific Laplacian kernel used in the experiment.

---

## Question 3

Suppose PSNR improves after averaging, but a human observer considers the 9×9 result worse for navigation.
...
* Edge preservation
* Difference between numerical similarity and task-specific image quality

---

## Question 4

Increase the Gaussian noise level beyond:

```text
σ = 25
```

and investigate at what point sharpening can no longer recover usable edges.

Explain what this demonstrates about the limitations of spatial-domain preprocessing.

Also discuss what additional processing may be required beyond simple spatial filtering.
...
**Unit:** Spatial Filtering
**Experiment:** Restoring and Enhancing Degraded Onboard Camera Feed for an Autonomous Ground Vehicle (AGV)
